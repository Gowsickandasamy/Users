...
public class UserService {
	...
	public List<User> saveUser(MultipartFile file) throws EncryptedDocumentException, IOException {
		...
		rows = StreamSupport.stream(sheet.spliterator(), false).skip(1).map(row -> StreamSupport
		...
		for (List<String> row : rows) {
			...
			if (existingUser == null) {
				...
				throw new UserManagementException(
						"User with employee ID " + excelData.getEmployeeId() + " is already exists");
			}
		}
	...
	public List<Long> findUserEmployeeIds() {
		...
		return trainees.stream().map(User::getEmployeeId).collect(Collectors.toList());
	}
	...
	public List<Long> findEmployeeIdsByBusinessUnit(String businessUnit) {
		...
		return users.stream().map(User::getEmployeeId).collect(Collectors.toList());
	}
	...
	public List<UserDTO> getTrainer() {
		...
		return trainees.stream().map(this::mapToUserDTO).collect(Collectors.toList());
